...
//查询数据
bool QueryDatabase(const string& table,const string& a,const string& b,const string& c,string& d){

       char query[150]; //查询语句
      //将数据格式化输出到字符串
       sprintf(query, "select %s from %s where %s='%s'",a.c_str(),table.c_str(),b.c_str(),c.c_str());
      //设置编码格式
...
bool QueryDatabase(const string& table, vector<vector<string> > & data){
       ...
       sprintf(query, "select * from %s where zd='0' union all select * \
           from %s where zd<>'0';",table.c_str(),table.c_str());
...
bool QueryDatabase2(const string& table, vector<vector<string> > & data){

       char query[150]; //查询语句
       //sprintf(query, "select * from %s",table.c_str());
       sprintf(query, "select message from %s where zd='0' union all select * \
           from %s where zd<>'0';",table.c_str(),table.c_str());
...
bool QueryDatabase3(const string& query, vector<vector<string> > & data){
...
//插入数据
bool InsertData(const string& table, const string& username, const string& password,int ret = 0){

    char query[150];    //查询语句
    mysql_query(&mysql, "SET NAMES UTF8"); 
    sprintf(query, "insert into %s(username,password,access) values('%s','%s','%d');",table.c_str(),username.c_str(),password.c_str(),ret);
    if (mysql_query(&mysql, query)) {
           return false;
    }
    else{
...
bool InsertData3(const string& query){
